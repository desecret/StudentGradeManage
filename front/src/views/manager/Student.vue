<template>
  <div>
    ...
    <div class="card" style="margin-bottom: 5px">
      <el-input style="width: 260px; margin-right: 10px" v-model="data.name" placeholder="请输入姓名进行查询" :prefix-icon="Search" @keyup.enter="load"/>
      <el-input style="width: 260px; margin-right: 10px" v-model="data.studentId" placeholder="请输入学号进行查询" :prefix-icon="Search" @keyup.enter="load"/>
    ...
    <div class="card" style="margin-bottom: 5px">
      ...
      <div>
        <el-table :data="data.tableData" style="width: 100%">
          ...
          <el-table-column prop="belongClassId" label="所属班级" sortable/>

          <el-table-column>
            <template #default="scope">
              <el-button type="primary" @click="handelEdit(scope.row)">编辑</el-button>
              <el-button type="danger" @click="handelDelete(scope.row.studentId)">删除</el-button>
            </template>
          </el-table-column>
        </el-table>
      </div>
    ...
    <el-dialog v-model="data.FormVisible" title="学生信息" width="35%">
...
<!--        </el-form-item>-->
        ...
        <el-form-item label="所属班级编号">
          <el-input v-model="data.form.belongClassId" autocomplete="off" />
        </el-form-item>
      </el-form>
...
//拿到分页数据
const load = () => {
  request.get('http://localhost:8080/student/page',{
    params: {
...
const saveCourse = () => {
  ...
  if(data.form.studentId === ''){
    request.post("http://localhost:8080/student/insert",data.form).then(res => {
      console.log(res)
      if(res === 201){
    ...
    request.post('http://localhost:8080/student/update',data.form).then(res => {
      if(res === 201) {
        ElMessage.error('没有此班级')
      } else if(res === 202) {
...
const handelDelete = (studentId) => {
  ...
  ElMessageBox.confirm('删除后无法恢复，您确认删除吗？', '删除确认', {type : 'warning'}).then(res => {
    request.post('http://localhost:8080/student/delete',studentId).then(res => {
      if(res === 201) {
