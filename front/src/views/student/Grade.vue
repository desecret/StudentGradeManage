<template>
  <div>
    ...
    <div class="card" style="margin-bottom: 5px">
      <el-input style="width: 260px; margin-right: 10px" v-model="data.courseName" placeholder="请输入课程名进行查询" :prefix-icon="Search" @keyup.enter="load"/>
      <el-input style="width: 260px; margin-right: 10px" v-model="data.courseCode" placeholder="请输入课程编号进行查询" :prefix-icon="Search" @keyup.enter="load"/>
...
//拿到分页数据
const load = () => {
  console.log(JSON.parse(localStorage.getItem('student-user-id') || '{}'))
  request.get('http://localhost:8080/grade/id',{
    params: {
      pageNum: data.pageNum,
      pageSize:data.pageSize,
      studentId:JSON.parse(localStorage.getItem('student-user-id') || '{}'),
